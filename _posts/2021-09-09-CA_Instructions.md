...
2의 보수를 모른다면 풀 수 없을 것이다. 저걸 언제 다 계산해서 정답을 작성 할 수 있을까.. 당연히 2의 보수를 사용하면 쉽게 풀 수 있다.
...
**ISA**는 **HW와 SW간의 추상적인 인터페이스**로 기계어를 작성하기 위해 필요한 **모든 정보**를 말하며 단순히 Instructions의 집합뿐만 아니라 **register, memory, access** 등을 포함한 모든 정보를 의미한다. 
...
RISC와 CISC는 컴퓨터 아키텍처의 두 가지 주요 접근 방식이다. 이 둘의 차이점을 간략히 살펴보자.

-   **RISC (Reduced Instruction Set Computer)**: 명령어 집합을 최소화하여 단순하고 빠른 성능을 목표로 한다. 주로 사용되는 명령어만 포함하여 설계가 간단하고 효율적이다. 일반적으로 파이프라이닝 기법을 사용하여 명령어를 더 빠르게 실행한다.
    
-   **CISC (Complex Instruction Set Computer)**: 많은 수의 명령어를 포함하여 복잡한 작업을 직접 수행할 수 있도록 설계되었다. 명령어 해석에 시간이 걸리고 회로가 복잡하지만, 한 번의 명령어로 복잡한 작업을 수행할 수 있다.
...
컴퓨터 구조에서 명령어 세트는 CPU와 메모리 간의 중요한 인터페이스를 제공하며, 이를 통해 소프트웨어는 하드웨어와 상호작용할 수 있다. RISC와 CISC는 이러한 명령어 세트의 구현 방식에 따라 다른 접근 방식을 취하고 있으며, 각각의 장단점이 존재한다. 컴퓨터 아키텍처를 이해하는 데 있어 이러한 차이점을 이해하는 것은 매우 중요하다.
